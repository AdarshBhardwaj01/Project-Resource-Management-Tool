...
public class ManageEmployeeSkillsScreen
{
    ...
    private static void DisplaySkillsScreen(string fullName, IReadOnlyList<EmployeeSkillDto> skills)
    {
        ConsoleHelper.WriteHeader("Manage Skills");
    ...
    private async Task AddSkillAsync(int employeeId)
    {
        ...
        try
        {
            var message = await _employeeApiClient.AddEmployeeSkillAsync(employeeId, new AddEmployeeSkillRequest
            {
                SkillName = skillName,
    ...
    private async Task UpdateSkillAsync(int employeeId, IReadOnlyList<EmployeeSkillDto> skills)
    {
        if (skills.Count == 0)
        {
        ...
        try
        {
            var message = await _employeeApiClient.UpdateEmployeeSkillAsync(
                ...
                new UpdateEmployeeSkillRequest { ProficiencyLevel = int.Parse(proficiencyChoice) });
    ...
    private async Task RemoveSkillAsync(int employeeId, IReadOnlyList<EmployeeSkillDto> skills)
    {
        if (skills.Count == 0)
        {
