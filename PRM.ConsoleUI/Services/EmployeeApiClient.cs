...
public class EmployeeApiClient : ApiClientBase
{
    ...
    public async Task<string> CreateEmployeeAsync(CreateResourceRequest request)
    {
    ...
    public async Task<ResourceListResponse> GetAllEmployeesAsync(string? status = null, string? department = null)
    {
        ApplyAuthorizationHeader();
        ...
        return await ReadJsonAsync<ResourceListResponse>(response)
            ?? new ResourceListResponse();
    }

    public async Task<ResourceDetailDto> GetEmployeeAsync(int id)
    {
        ApplyAuthorizationHeader();
        var response = await HttpClient.GetAsync($"api/employees/{id}");
        await EnsureSuccessAsync(response, "Employee not found.");
        return await ReadJsonAsync<ResourceDetailDto>(response)
            ?? throw new InvalidOperationException("Employee not found.");
    }

    public async Task<string> UpdateEmployeeAsync(int id, UpdateResourceRequest request)
    {
        ApplyAuthorizationHeader();
    ...
    public async Task<IReadOnlyList<ResourceSkillDto>> GetEmployeeSkillsAsync(int employeeId)
    {
        ApplyAuthorizationHeader();
        ...
        return await ReadJsonAsync<List<ResourceSkillDto>>(response)
            ?? new List<ResourceSkillDto>();
    }

    public async Task<string> AddEmployeeSkillAsync(int employeeId, AddResourceSkillRequest request)
    {
        ApplyAuthorizationHeader();
    ...
    public async Task<string> UpdateEmployeeSkillAsync(int employeeId, int skillId, UpdateResourceSkillRequest request)
    {
        ApplyAuthorizationHeader();
