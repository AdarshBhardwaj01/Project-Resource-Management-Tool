...
public class ProjectService : IProjectService
{
    ...
    public async Task<string> CreateProjectAsync(CreateProjectRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await _employeeStatusSchedulerService.RecomputeResourceStatusByUserIdAsync(
            request.ManagerId,
            cancellationToken);
        await _projectRepository.SaveChangesAsync(cancellationToken);
    ...
    {
        var manager = await _userRepository.FindByUsernameOrIdAsync(managerId.ToString(), cancellationToken);
        if (manager is null || !manager.IsActive)
        {
            throw new BusinessValidationException("Manager ID does not exist or is inactive.");
        }
        if (!await _userRepository.HasRoleAsync(managerId, RoleNames.Manager, cancellationToken))
        {
